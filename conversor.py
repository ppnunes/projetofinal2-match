...
class ConversorMoedas:
    # API used to get rate
    _API_URL_ = "https://economia.awesomeapi.com.br/last"
    ...
    _codes_ = set(
        [
            i
            for row in map(lambda x: x.split("-"), _codes_avaialble_.keys())
            for i in row
        ]
    )

    def _test_rate_with_usd(self, code: str, codein: str) -> float:
        code1 = f"{code}-USD"
        code2 = f"USD-{codein}"
        if (
            code1 in self._codes_avaialble_.keys()
            and code2 in self._codes_avaialble_.keys()
        ):
            return self.get_rate(code, "USD") * self.get_rate("USD", codein)
        else:
            return 0

    def all_codes(self) -> set:
        """Show all codes available"""
        return ", ".join(sorted(self._codes_))

    def all_conversions(self) -> list:
        """Show all direct conversions"""
        return ", ".join(sorted(self._codes_avaialble_.keys()))

    def is_available(self, code: str) -> bool:
        """Check if code is available"""
        return code.upper() in self._codes_

    def get_rate(self, code: str, codein: str) -> float:
        """Get the rate of two codes"""
        url = f"{self._API_URL_}/{code.upper()}-{codein.upper()}"
        response = get(url)
        if response.status_code != 200:
            raise Exception(response.json().get("code"))
        data = response.json()
    ...
    def convert(
        ...
        if f"{code.upper()}-{codein.upper()}" not in self._codes_avaialble_:
            if self._test_rate_with_usd(code, codein) > 0 and use_usd:
                print(
                    "Conversão direta indisponível, utilizando USD como conversão intermediária"
                )
